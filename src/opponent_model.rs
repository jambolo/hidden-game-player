...
impl OpponentModel {
    pub fn predict_move_probability(&self, possible_moves: &[Move], game_state: &GameState) 
        -> HashMap<Move, f64> {
        let mut probabilities = HashMap::new();
        
        for game_move in possible_moves {
            let move_type = self.classify_move(game_move, game_state);
            let base_prob = self.move_preferences.get(&move_type).unwrap_or(&0.5);
            
            // Adjust based on game state and playing style
            let adjusted_prob = self.adjust_for_context(*base_prob, game_move, game_state);
    ...
    pub fn update_from_move(&mut self, game_move: &Move, game_state: &GameState) {
        let move_type = self.classify_move(game_move, game_state);
