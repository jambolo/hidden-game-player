...
impl InformationState {
    pub fn update_on_opponent_move(&mut self, game_move: &Move, game_state: &GameState) {
        // Update known tiles
        ...
        self.update_hand_probabilities(game_move, game_state);
        
        // Infer new constraints
        self.infer_constraints(game_move, game_state);
    }
    
    pub fn update_on_opponent_pass(&mut self, game_state: &GameState) {
        // Opponent passed - they don't have any playable tiles
        let playable_numbers = game_state.get_playable_numbers();
