...
impl InformationSetMCTS {
    pub fn simulate(&mut self, state: &GameState, player_color: Color) -> f64 {
    ...
    fn select_and_expand(&mut self, state_key: &GameStateKey, state: &GameState) 
        -> (GameState, Vec<GameStateKey>) {
        let mut current_state = state.clone();
        let mut path = vec![state_key.clone()];
        
        loop {
            let node = self.tree.entry(state_key.clone())
